...
mod tests {
    ...
    #[tokio::test]
    async fn test_scan_populates_scan_time() {
        let config = test_config();
        let engine = ReconnaissanceEngine::new(&config);
    ...
    #[tokio::test]
    async fn test_scan_discovers_self() {
        let config = test_config();
        let engine = ReconnaissanceEngine::new(&config);
        let scan = engine.scan().await.unwrap();
        assert!(scan.nodes.iter().any(|n| n.node_type == "skunkBat"));
    }

    #[tokio::test]
    async fn test_local_discovery_always_returns_self() {
        let discovery = LocalDiscovery;
        let nodes = discovery
            .discover_by_capability("nonexistent-capability")
            .await
            .expect("discovery should succeed");
        assert_eq!(nodes.len(), 1, "local discovery always returns self");
        assert_eq!(nodes[0].node_type, "skunkBat");
    }
    ...
    #[test]
    fn test_network_scan_serialization() {
        let scan = NetworkScan {
            ...
            topology: vec![],
            scope: NetworkScope::default(),
            scan_time: Some(SystemTime::UNIX_EPOCH),
        };
    ...
    #[test]
    fn test_network_scope_serialization() {
        let scope = NetworkScope {
            owned_networks: vec!["192.168.1.0/24".to_string()],
            managed_systems: vec!["server1".to_string()],
            excluded: vec!["192.168.1.255".to_string()],
        };
        let json = serde_json::to_string(&scope).unwrap();
        let parsed: NetworkScope = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.owned_networks.len(), 1);
        assert_eq!(parsed.excluded.len(), 1);
    }

    #[test]
    fn test_local_node_has_all_capabilities() {
        let node = LocalDiscovery::local_node();
        assert!(node.capabilities.contains(&"reconnaissance".to_string()));
        assert!(node.capabilities.contains(&"defense".to_string()));
        assert!(node.capabilities.contains(&"threat-detection".to_string()));
        assert!(node.capabilities.contains(&"observability".to_string()));
    }
    ...
    #[tokio::test]
    async fn test_mapper_with_nodes() {
        let mapper = SimpleTopologyMapper;
        let nodes = vec![
            Node {
                id: "n1".to_string(),
